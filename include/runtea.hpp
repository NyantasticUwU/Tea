...
namespace tea
{
    ...
    void runTea(const std::vector<std::string> &teafile, const char *&filename,
        const std::string &currentNamespace, const teaString_t *const &pteaStrings = nullptr,
        const teaInt_t *const &pteaInts = nullptr, const teaFloat_t *const &pteaFloats = nullptr,
        const teaSnippet_t *const &pteaSnippets = nullptr, const teaArray_t *const &pteaArrays = nullptr);
